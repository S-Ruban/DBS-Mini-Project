...
SELECT OrderNo AS Order_No, OrderTime As Order_Time, isPaid AS isCompleted, Rest_Name AS Restaurant, FirstName AS Delivery, SUM(Quantity) AS Quantity, (SUM(Quantity*Price) + Del_Charge) AS Price
FROM ((((SELECT * FROM ORDERS WHERE Cust_Uname = {uname}) AS CUSTOMER 
        NATURAL JOIN ORDER_CONTENTS) 
        NATURAL JOIN FOOD_ITEMS) 
        NATURAL JOIN RESTAURANTS) 
        JOIN USERS ON Del_Uname = Uname 
GROUP BY ( Order_No, Order_Time, isCompleted, Restaurant, Delivery, Del_Charge)
ORDER BY Order_Time DESC;
...
SELECT OrderNo AS Order_No, OrderTime AS Order_Time, isPaid  AS isCompleted, Cust_FirstName AS Customer, Del_FirstName AS Delivery, SUM(Quantity) AS Quantity, (SUM(Quantity*Price) + Del_Charge) AS Price 
FROM (((SELECT * FROM ORDERS WHERE FSSAI = {fssai}) AS RESTAURANT 
        NATURAL JOIN ORDER_CONTENTS) 
        NATURAL JOIN FOOD_ITEMS) 
...
GROUP BY ( Order_No, Order_Time, isCompleted, Customer, Delivery, Del_Charge)
ORDER BY Order_Time DESC;
...
SELECT OrderNo AS Order_No, OrderTime AS Order_Time, isPaid  AS isCompleted, FirstName AS Customer, Rest_Name AS Restaurant, SUM(Quantity) AS Quantity, Del_Charge AS Price
FROM ((((SELECT * FROM ORDERS WHERE Del_Uname = '{uname}') AS DELIVERY_PERSON 
        NATURAL JOIN ORDER_CONTENTS) 
        NATURAL JOIN FOOD_ITEMS) 
        NATURAL JOIN RESTAURANTS) 
        JOIN USERS ON Cust_Uname = Uname
GROUP BY ( Order_No, Order_Time, isCompleted, Customer, Restaurant, Del_Charge)
ORDER BY Order_Time DESC;
